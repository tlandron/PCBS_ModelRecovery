...
for isubj = 1:nsubj
    ...
    for itask = 1:2
        ...
        prev_fit(isubj,itask) = out_fit.prev;
        beta_fit(isubj,itask) = out_fit.beta;

    end
end

%%
% from now on, fix prev = 0.225 (average across subjects and conditions)
%                  beta = 1.250

prev_fix = 0.225; % one run of mean(prev_fit(:))
beta_fix = 1.250; % one run of mean(beta_fix(:))

nsim = 1e3; % number of simulations

prev_sim = nan(nsubj,2,nsim);
beta_sim = nan(nsubj,2,nsim);
...
%         
%         
%         cfg_fit.raft = out_sim.raft;
%         cfg_fit.rbef = out_sim.rbef;
%         out = fit_model_softmax(cfg);
%         
%         prev_sim(isubj,itask,1) = out.prev;
%         beta_sim(isubj,itask,1) = out.beta;
%         
%         error
